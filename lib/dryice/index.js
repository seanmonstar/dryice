...
function CommonJsProject(opts) {
    ...
    this.currentFiles = {};
    this.ignoredFiles = {};
    this.ignores = opts.ignoreErrors || [];
}
...
(function() {
    ...
    function findRequires(baseObj) {
        var pathname = baseObj.base + baseObj.path;
        var canExecute = false;
        try {
            canExecute = !!require(pathname);
        } catch (cantExecute) {
    ...
    CommonJsProject.prototype.require = function(id, parent) {
        ...
        this.roots.forEach(function(root) {
            ...
            if (textPluginPattern.test(module)) {
                ...
                function tryExtensions(module) {
                    var mod;
                    for (var ext in require.extensions) {
                        mod = findModuleAt(baseObj, base, module + ext);
                        if (mod) {
                            return mod;
                        }
                    }
                }
                if (path.extname(module) in require.extensions) {
                    baseObj = findModuleAt(baseObj, base, module);
                } else {
                    baseObj = tryExtensions(module);
                    if (!baseObj) {
                        baseObj = tryExtensions(path.join(module,'/index'));
                    }
                }
            }
        }, this);
    ...
    CommonJsProject.prototype.ignoreModule = function(id) {
        return this.ignores.some(function(i) {
            return id.indexOf(i) !== -1;
        });
    };
})();
